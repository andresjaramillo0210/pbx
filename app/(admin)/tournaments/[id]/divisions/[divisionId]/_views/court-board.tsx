...
export default function AdminCourtBoardView() {
  ...
  const goToScore = (matchId: string) => {
    router.push({
      pathname: '/(admin)/tournaments/[id]/divisions/[divisionId]/matches/[matchId]/score',
  ...
  return (
    <ScreenContainer maxWidth={1400}>
      ...
      {/* Back link to the admin division page. */}
      <Pressable
        onPress={goBack}
        accessibilityRole="link"
        accessibilityLabel="Back to division"
...
// Admin: tap a row to open the dedicated score screen.
...
function MatchRow({
  ...
  return (
    <Pressable
      onPress={onPress}
      accessibilityRole="button"
      accessibilityLabel={`Score ${aName} vs ${bName}`}
      style={({ pressed, hovered }: { pressed: boolean; hovered?: boolean }) => [
        containerStyles,
...
const styles = StyleSheet.create({
  ...
  rowHover: { borderColor: colors.borderStrong },
